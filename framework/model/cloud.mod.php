...
define('CLOUD_GATEWAY_URL', 'https://ss.edfp.top/gateway.php');
define('CLOUD_GATEAUTH_URL', base64_decode('L2FwcC93YXBpL2dhdGVhdXRo'));
define('CLOUD_GATEWAY_URL_NORMAL', 'http://ss.edfp.top/gateway.php');
...
function cloud_build() {
	...
	$dat = cloud_request('http://ss.edfp.top/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://ss.edfp.top/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://ss.edfp.top/gateway.php', $pars);
	if (is_error($response)) {
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://ss.edfp.top/gateway.php', $pars);
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://ss.edfp.top/gateway.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
