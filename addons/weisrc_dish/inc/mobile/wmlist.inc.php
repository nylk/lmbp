...
//var_dump($_COOKIE[$this->_auth2_openid]);exit;
...
//$nickname = "人民币";
//$headimgurl = "http://wx.qlogo.cn/mmopen/vi_32/p6Xde5fS02dokEtoPDoVSgSzUC9a1rZibPw0wrxjDdyJHcRGhBTVZHtfZG2nuoCW8NqpT3NHRiaAjEPS3ESP2nCQ/132";
...
$label = array_column($label, 'title', id);

//ini_set('display_errors', true);
//error_reporting(E_ALL ^ E_WARNING ^ E_NOTICE ^ E_STRICT ^ E_USER_WARNING);
...
foreach ($cart as $key => $value) {
    ...
    $totalprice = floatval($totalprice + $value['total'] * $value['price']);
    if ($value['marketprice']) {
        $marketprice = floatval($marketprice + $value['total'] * $value['marketprice']);
    } else {
        $marketprice = floatval($marketprice + $value['total'] * $value['price']);
    }

    $cart[$key]['totalprice'] = floatval($value['total'] * $value['price']);
    $cart[$key]['totalmarketprice'] = floatval($value['total'] * $value['marketprice']);

    $cartgoods[$value['goodsid']]['total'] = $cartgoods[$value['goodsid']]['total'] + $value['total'];
}

$discount = floatval($marketprice - $totalprice);
...
$share_url = $host . 'app/' . $this->createMobileUrl('wmlist', array('storeid' => $storeid, 'mode' => $mode, 'tablesid' => $tablesid, 'agentid' => $fans['id']), true);
...
include $this->template($this->cur_tpl . '/wmlist');
