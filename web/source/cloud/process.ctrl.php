...
if ($step == 'files' && $_GPC['m'] == 'prepare') {
        ...
        if($return == 5858){
            itoast('未授权域名，请联系客服QQ: ',create_url('cloud/upgrade',array('op' => 'display')),'error');
            die;
        }
        if($return == 7474){
            itoast('授权IP错误，请联系客服QQ:  ',create_url('cloud/upgrade',array('op' => 'display')),'error');
            die;
        }
        if($return == 6161){
            itoast('授权已经到期，请联系客服QQ: ',create_url('cloud/upgrade',array('op' => 'display')),'error');
            die;
        } 
      	$upgrade = $return;
...
if ($step == 'filespro') {
  ...
  $newver = "<?php return array ('family' => 'x','ver' => '$lastver','releasedate' => '$fbh');?>";
  $ver = fopen(IA_ROOT.'/data/version.php','w+','w+');
  fwrite($ver,$newver);
  fclose($ver);
  if($packet['sqls']){
